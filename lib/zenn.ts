/**
 * Zenn記事同期ロジック（CLI / Vercel Cron 共通）
 *
 * Zenn公開APIから記事一覧・詳細を取得し、Neon DBへupsertする。
 * 副作用を持たない純関数として実装し、CLI（sync_zenn.ts）と
 * Vercel Cronのエンドポイント（api/cron/sync-zenn.ts）の双方から呼び出す。
 *
 * 環境変数:
...
/**
 * Zenn記事をNeon DBへ全件同期する。
 * @returns 取得件数・成功件数・失敗件数のサマリー
 */
export async function syncZenn(): Promise<SyncResult> {
  ...
  if (allArticles.length === 0) {
    ...
    return { platform: "zenn", total: 0, synced: 0, errors: 0 };
  }
