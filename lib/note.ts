/**
 * note記事同期ロジック（CLI / Vercel Cron 共通）
 *
 * note非公式API（v2/v3）から記事一覧・詳細を取得し、Neon DBへupsertする。
 * 副作用を持たない純関数として実装し、CLI（sync_note.ts）と
 * Vercel Cronのエンドポイント（api/cron/sync-note.ts）の双方から呼び出す。
 *
 * 環境変数:
...
/**
 * note記事をNeon DBへ全件同期する。
 * @returns 取得件数・成功件数・失敗件数のサマリー
 */
export async function syncNote(): Promise<SyncResult> {
  ...
  if (allArticles.length === 0) {
    ...
    return { platform: "note", total: 0, synced: 0, errors: 0 };
  }
